...
define(function(require,exports){
	...
	var collectionConfig = function(){
		return {
			lines: 1,
			key: 'syy_rx8',
			localStorage: new Store('syyrx8ball'),
			deviation: -1,
			/**
			 * 验证是否符合此玩法的选号规则
			...
			getBetArray: function(){
				...
				return {
					...
					bet: Tool.numC(selectArray[0].length,8),
	...
	var appConfig = function(isManualFirst,collection){
		return {
			...
			random: function(){
				...
				var l1 = Tool.baseBallRandom(8,11,false,true,'ceil').sort();
	...
	return {
		initialize: function(isManualFirst){
			...
			C.syyrx8ballcollection = new Ball.Collection(collectionConfig());
			Tool.detectLocalData('syyrx8ball',C.syyrx8ballcollection);
			C.syyrx8ballapp = new Ball.App(appConfig(isManualFirst,C.syyrx8ballcollection));
		}
	};
});
