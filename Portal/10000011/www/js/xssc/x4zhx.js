...
define(function(require,exports){
	...
	var collectionConfig = function(){
		return {
			lines: 4,
			key: 'xssc_x4zhx',
			localStorage: new Store('xsscx4zhxball'),
			deviation: 0,
			/**
			 * 验证是否符合此玩法的选号规则
			...
			getBetArray: function(){
				...
				return {
					...
					bet: selectArray[0].length * selectArray[1].length * selectArray[2].length * selectArray[3].length,
	...
	return {
		initialize: function(isManualFirst){
			...
			C.xsscx4zhxballcollection = new Ball.Collection(collectionConfig());
			Tool.detectLocalData('xsscx4zhxball',C.xsscx4zhxballcollection);
			C.xsscx4zhxballapp = new Ball.App(appConfig(isManualFirst,C.xsscx4zhxballcollection));
		}
	};
});
